...
class Evolution implements IMetaheuristics {
    ...
    Evolution(Configuration configuration) {
        this.popSize = configuration.getPopSize();
        this.numOfGeners = configuration.getNumOfGeners();
        this.avoidClones = configuration.isAvoidClones();
        this.mutation = configuration.getMutationType();
        this.crossingOver = configuration.getCrossingOver();
...
//        appendParetoFrontToStringBuilder(sBLastPopFront, archive);
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            while (attemptsLeft > 0) {
                if (population.contains(children[i])) {
                    mutation.mutate(children[i], 1f);//przeszukaj lokalnie przestrzeń dookoła
                    attemptsLeft--;
                    howManyClones++;
                } else {
                    break;
    ...
    private Individual_NSGA_II[] matingPool(int generation) {
        Individual_NSGA_II[] children = crossingOver.crossOver(selection.select(population), selection.select(population), generation);
